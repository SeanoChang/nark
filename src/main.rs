...
fn main() -> Result<()> {
    ...
    match args.command {
        Init => run(&vault_dir),
        Jot { author, domain, intent, kind, status, title, tag, body, from } => cli::jot::run(&vault_dir, title, &author, domain.as_deref(), kind.as_deref(), intent.as_deref(), status.as_deref(), &tag, body.as_deref(), from.as_deref()),
